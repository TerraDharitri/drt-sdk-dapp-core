...
import { DrtUnlockPanel } from 'lib/sdkDappCoreUi';
...
export class UnlockPanelManager {
  ...
  private unlockPanelElement: DrtUnlockPanel | null = null;
  ...
  private async ensureUnlockPanelElementExists() {
    if (!this.unlockPanelElement) {
      this.unlockPanelElement = await createUIElement<DrtUnlockPanel>({
